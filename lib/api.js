...
function extractPictures(fetchResponse){
  ...
  res.forEach(item => {
    returnArray.push({url: item?.projectImagesCollection?.items[0]?.url, slug: item?.slug, title: item?.title, bannerImage: item?.bannerImage, tags: item?.contentfulMetadata?.tags, homesort: item?.contentfulMetadata?.tags.find((element) => {
...
export async function getProjectAndMoreProjects(slug) {
  ...
  const entries = await fetchGraphQL(
    `query {
      projectCollection(where: { slug_not_in: "${slug}" }, limit: 2) {
        items {
          ${PROJECT_GRAPHQL_FIELDS}
        }
      }
